...
static void set_error(char *error, size_t error_size, const char *message, const char *detail, int line_number) {
  if (error == NULL || error_size == 0U) {
    return;
  }
  if (line_number > 0) {
    (void)snprintf(error, error_size, "line %d: %s%s%s", line_number, message, detail ? ": " : "", detail ? detail : "");
  } else {
    (void)snprintf(error, error_size, "%s%s%s", message, detail ? ": " : "", detail ? detail : "");
  }
}
...
int sitl_initial_conditions_load(const char *path, sitl_initial_conditions_t *initial, char *error, size_t error_size) {
  FILE *file;
