                                          ...
                                          size_t error_size)
{
    ...
    if (!real_is_finite(vehicle->min_airspeed_mps) || !real_is_finite(vehicle->max_airspeed_mps) ||
    ...
    if (!real_is_finite(vehicle->min_actuator) || !real_is_finite(vehicle->max_actuator) ||
        vehicle->min_actuator < -1.0f || vehicle->max_actuator > 1.0f ||
        vehicle->min_actuator >= vehicle->max_actuator)
    {
        return fail_validation(
            error, error_size, "vehicle actuator range must fit normalized sim outputs");
    }
    ...
    {
        return fail_validation(
            error, error_size, "sim geometry and thrust must be finite and positive");
    }
    ...
    {
        return fail_validation(
            error, error_size, "safe actuator outputs violate vehicle/sim ranges");
    }
