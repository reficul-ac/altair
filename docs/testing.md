...
The Qt capture launches the built `animus_qt` executable once per workspace. If `DISPLAY` is already set, the capture uses that display unchanged. If no display is present, the helper uses `xvfb-run`; constrained environments without a usable `xvfb-run` fall back to Qt offscreen rendering with the software backend, and the manifest records that fallback. It captures `map-2d`, `terrain-3d`, and `setup` with deterministic mock telemetry, writes PNGs under `artifacts/animus-qt-screenshots/<timestamp>/screenshots/`, records per-workspace logs, and fails when a PNG is missing, zero-sized, visually blank, the wrong size, or too similar to another workspace capture. Under `xvfb-run`, the helper enables Qt WebEngine WebGL flags so `terrain-3d` exercises the real bundled Cesium path and writes the native Cesium canvas PNG. Terrain 3D capture also injects a known deflected control-surface snapshot and records a WebEngine diagnostic that fails if any required GLB pivot node is missing or remains at its neutral matrix. `visual-report.md` and `run-manifest.json` include the control-surface diagnostic, region diagnostics for toolbar/tab visibility, workspace content variation, and color diversity warnings. The `--no-run` flag remains available for dependency-light readiness checks only.
