...
The capture workflow starts a no-QGC live SITL session, opens Animus through Electron, captures the `flight`, `dashboard`, `map`, `inspector`, `plan`, `setup`, and `video` workspaces with a requested `1440x900` Electron outer window, and writes screenshots, logs, `run-manifest.json`, the Electron `manifest.json`, and `visual-report.md` under `artifacts/animus-screenshots/<timestamp>/`. The saved PNG dimensions reflect Electron's web contents area after the window frame is allocated; use that as the current visual contract unless a future capture mode explicitly requests web-contents sizing.

Inspect `visual-report.md` first for capture status, live telemetry, workspace/panel mismatches, renderer state, and screenshot diagnostics, then inspect the generated screenshots before review. Check that the 3D scene is nonblank, live link/telemetry state is visible, the selected workspace matches the file name, desktop layout is usable, and controls/text are not obviously clipped or overlapping. The Map workspace must show selected offline satellite imagery behind vehicle overlays when an active licensed tile cache is configured; an offline-satellite-cache-unavailable state is acceptable when intentionally testing a missing cache. When an active DEM cache is configured, also check that `Terrain 3D` is enabled and that Flight View terrain remains visible with shaded terrain if satellite imagery is unavailable. This workflow is required for Animus UI/layout changes even before it becomes a GitHub Actions status check; if it cannot run, note the reason and the residual visual risk in the review or final response.
