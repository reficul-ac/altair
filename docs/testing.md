...
Inspect `visual-report.md` first for capture status, live telemetry, workspace/panel mismatches, renderer state, and screenshot diagnostics, then inspect the generated screenshots before review. Check that the 3D scene is nonblank, live link/telemetry state is visible, the selected workspace matches the file name, desktop layout is usable, and controls/text are not obviously clipped or overlapping. The Map workspace must show selected offline satellite imagery behind vehicle overlays when a licensed map pack is configured; an offline-map-unavailable state is acceptable when intentionally testing missing or unreadable satellite/terrain PMTiles paths. This workflow is required for Animus UI/layout changes even before it becomes a GitHub Actions status check; if it cannot run, note the reason and the residual visual risk in the review or final response.
