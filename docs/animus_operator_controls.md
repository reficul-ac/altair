...
- Live bridge snapshots include additive `fieldStates` metadata for documented groups. Existing top-level values remain unchanged; clients can treat `fresh` as updated within the contract timeout, `stale` as older than the timeout, `unsupported` as unavailable from the current source contract, and `unknown` as supported but not sampled yet.
- `mavlink_live_bridge.py --record-altlog <path>` records received raw MAVLink v1 packets to an `.altlog` bundle on shutdown. `tools/python/export_tlog.py` exports those packet records as an exact raw `.tlog` byte stream.
