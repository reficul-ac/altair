#!/usr/bin/env python3
"""Run Playwright interactions against Animus backed by live SITL."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--keep-going", action="store_true", help="write manifest and return success on failure"
    )
...
def require_tools(root: pathlib.Path, install_deps: bool) -> None:
    if shutil.which("npm") is None:
        raise RuntimeError("npm is required for Animus interaction tests")
...
def playwright_command(command: list[str], headed: bool) -> list[str]:
    if headed and not os.environ.get("DISPLAY"):
        xvfb_run = shutil.which("xvfb-run")
        if xvfb_run is None:
            raise RuntimeError("headed Playwright requires DISPLAY or xvfb-run")
        return [xvfb_run, "-a", *command]
    return command


def collect_screenshots(out_dir: pathlib.Path) -> list[str]:
    screenshot_dir = out_dir / "screenshots"
    if not screenshot_dir.exists():
        return []
    return [str(path) for path in sorted(screenshot_dir.glob("*.png"))]
...
def run(args: argparse.Namespace) -> int:
    ...
    out_dir = (
        ...
        if args.output_dir
        else root / "artifacts" / "animus-interactions" / timestamp()
    )
    ...
    manifest = {
        ...
        "logs": {},
        "ports": {},
        "playwrightReport": str(out_dir / "playwright-report" / "index.html"),
        "playwrightResults": str(out_dir / "playwright-results"),
        "errors": [],
    }
    ...
    try:
        ...
        command = playwright_command(
            ["npm", "run", "test:e2e", "--", *(["--headed"] if args.headed else [])],
            args.headed,
        )
        interaction_log = out_dir / "playwright.log"
        manifest["logs"]["playwright"] = str(interaction_log)
        env = {
            **os.environ,
            "ANIMUS_BASE_URL": url,
            "ANIMUS_ARTIFACT_DIR": str(out_dir),
            "ANIMUS_VIEWPORT": args.viewport,
        }
        with interaction_log.open("w", encoding="utf8") as log_file:
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"Playwright interactions failed with code {result.returncode}; see {interaction_log}"
            )
        ...
        if not args.keep_going:
            print(f"interact_animus_sitl.py: {exc}", file=sys.stderr)
            return 1
        return 0
    finally:
        terminate(processes)
