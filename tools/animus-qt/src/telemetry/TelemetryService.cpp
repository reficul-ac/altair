...
TelemetryService::TelemetryService(VehicleModel *vehicle, BreadcrumbPathModel *trail, QObject *parent)
    : QObject(parent),
      m_vehicle(vehicle),
      m_trail(trail),
      m_socket(nullptr),
      ...
      m_uiRateHz(20),
      m_udpPort(14551),
      m_udpHost(QStringLiteral("127.0.0.1")),
      m_elapsedS(0.0) {
...
bool TelemetryService::running() const { return m_running; }

int TelemetryService::uiRateHz() const { return m_uiRateHz; }

void TelemetryService::setUiRateHz(int uiRateHz) {
    const int bounded = qBound(1, uiRateHz, 30);
    if (m_uiRateHz == bounded) return;
    m_uiRateHz = bounded;
    if (m_mockRunning) m_timer.start(1000 / m_uiRateHz);
    m_publishTimer.setInterval(1000 / m_uiRateHz);
    emit uiRateHzChanged();
}

quint16 TelemetryService::udpPort() const { return m_udpPort; }

void TelemetryService::setUdpPort(quint16 udpPort) {
    if (m_udpPort == udpPort) return;
    m_udpPort = udpPort;
    emit udpEndpointChanged();
}

QString TelemetryService::udpHost() const { return m_udpHost; }

void TelemetryService::setUdpHost(const QString &udpHost) {
    if (m_udpHost == udpHost) return;
    m_udpHost = udpHost;
    emit udpEndpointChanged();
}
...
bool TelemetryService::startUdpTelemetry() {
    ...
    if (!m_socket->bind(address, m_udpPort, QUdpSocket::ShareAddress | QUdpSocket::ReuseAddressHint)) {
...
void TelemetryService::applySample(const MavlinkTelemetrySample &sample) {
    ...
    if (sample.hasMissionCurrent) m_vehicle->setMissionSeq(sample.missionSeq);
