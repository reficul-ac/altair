...
uint8_t readU8(const unsigned char *payload, int offset)
{
...
QVector<MavlinkTelemetrySample> MavlinkDecoder::decodeDatagram(const QByteArray &datagram) const
{
    ...
    while (offset < size)
    {
        ...
        if (decodeFrame(bytes + offset, frameLength, &sample))
...
bool MavlinkDecoder::decodeFrame(const unsigned char *frame,
                                 int frameLength,
                                 MavlinkTelemetrySample *sample)
{
    ...
    if (stx == MavlinkV1Stx)
    {
        if (frameLength < 8 || payloadLength > frameLength - 8)
            return false;
    ...
    {
        if (frameLength < 12 || payloadLength > frameLength - 12)
            return false;
    ...
    if (!verifyChecksum(frame, checksumOffset, msgId))
        return false;
    ...
    case 33:
        ...
        sample->headingDeg = readU16(payload, 26) == 65535U
                                 ? 0.0
