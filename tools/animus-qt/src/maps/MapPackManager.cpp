...
bool MapPackManager::loadPack(const QDir &packDir, MapPack *pack, QString *error) const
{
    ...
    if (imageryFormat != QStringLiteral("xyz") && imageryFormat != QStringLiteral("mbtiles"))
    {
        if (error)
            *error =
                QStringLiteral("%1: imagery.format must be xyz or mbtiles").arg(packDir.dirName());
        return false;
    }
    ...
    pack->has2dImagery = imageryFormat == QStringLiteral("xyz") ||
                         QFileInfo::exists(packDir.filePath(QStringLiteral("2d/imagery.mbtiles")));
    pack->has3dTerrain =
        terrainFormat == QStringLiteral("quantized-mesh") &&
