  ...
  <main class="shell theme-grid">
    ...
    <section class="workspace-panel inspector-workspace" data-panel="setup">
      <div class="analysis-grid">
        ...
        <section><h2>Parameters / Diagnostics</h2><div class="inspector-actions"><input id="parameter-filter" type="search" placeholder="Filter parameters" /><button id="parameter-refresh" type="button">Refresh</button></div><div id="parameter-list" class="tool-list"></div><h2>Offline Satellite Cache</h2><div class="map-cache-form"><label><span>Provider</span><select id="map-cache-provider"><option value="altair-usgs">Altair default: USGS Imagery</option><option value="custom">Custom XYZ</option></select></label><label><span>XYZ URL</span><input id="map-cache-template" type="url" placeholder="https://tiles.example.com/{z}/{x}/{y}.png?key=..." /></label><label><span>Attribution</span><input id="map-cache-attribution" type="text" placeholder="Licensed imagery attribution" /></label><label><span>Set Label</span><input id="map-cache-label" type="text" placeholder="Field cache" /></label><label><span>Min Zoom</span><input id="map-cache-min-zoom" type="number" min="0" max="22" value="12" /></label><label><span>Max Zoom</span><input id="map-cache-max-zoom" type="number" min="0" max="22" value="16" /></label><label><span>Max Tiles</span><input id="map-cache-max-tiles" type="number" min="1" value="8000" /></label><label class="toggle map-cache-startup"><input id="map-cache-startup-default" type="checkbox" /><span>Use at startup</span></label></div><div class="inspector-actions"><button id="map-cache-estimate" type="button">Estimate</button><button id="map-cache-download" type="button">Cache</button><button id="map-cache-cancel" type="button">Cancel</button></div><div id="setup-map-cache-status" class="tool-list"></div><h2>Offline DEM Cache</h2><div class="map-cache-form"><label><span>Provider</span><select id="dem-cache-provider"><option value="altair-terrarium">Altair default: AWS Terrarium DEM</option><option value="custom">Custom XYZ</option></select></label><label><span>DEM XYZ URL</span><input id="dem-cache-template" type="url" placeholder="https://dem.example.com/{z}/{x}/{y}.png?key=..." /></label><label><span>Attribution</span><input id="dem-cache-attribution" type="text" placeholder="Licensed DEM attribution" /></label><label><span>Set Label</span><input id="dem-cache-label" type="text" placeholder="Field DEM" /></label><label><span>Encoding</span><select id="dem-cache-encoding"><option value="terrarium">Terrarium</option><option value="mapbox">Mapbox Terrain-RGB</option></select></label><label><span>Min Zoom</span><input id="dem-cache-min-zoom" type="number" min="0" max="22" value="12" /></label><label><span>Max Zoom</span><input id="dem-cache-max-zoom" type="number" min="0" max="22" value="15" /></label><label><span>Max Tiles</span><input id="dem-cache-max-tiles" type="number" min="1" value="8000" /></label><label class="toggle map-cache-startup"><input id="dem-cache-startup-default" type="checkbox" /><span>Use at startup</span></label></div><div class="inspector-actions"><button id="dem-cache-estimate" type="button">Estimate</button><button id="dem-cache-download" type="button">Cache</button><button id="dem-cache-cancel" type="button">Cancel</button></div><div id="setup-dem-cache-status" class="tool-list"></div><h2>Onboard Logs</h2><div class="inspector-actions"><button id="onboard-log-list" type="button">List</button><button id="onboard-log-erase" type="button">Erase</button></div><div id="onboard-log-listing" class="tool-list"></div><div id="diagnostics-list" class="tool-list"></div></section>
      </div>
    </section>
