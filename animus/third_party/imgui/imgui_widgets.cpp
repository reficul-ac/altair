    ...
    const ImRect &bb, ImGuiID id, bool *out_hovered, bool *out_held, ImGuiButtonFlags flags)
{
    ...
    if (hovered)
    {
        ...
        if (mods_ok)
        {
            if (mouse_button_clicked != -1 && g.ActiveId != id)
            {
                ...
                {
                    // FIXME: Traditionally ImGuiButtonFlags_PressedOnRelease never took ActiveId.
                    // Adding it in 2026-03-20 since ImGuiButtonFlags_NoHoldingActiveId can always
                    // be added. We don't yet perform an explicit ClearActiveID() to reduce scope of
                    // change, but this possibility could be investigated.
                    if (!(flags & ImGuiButtonFlags_NoHoldingActiveId))
                        SetActiveID(id, window); // Hold on ID
                    g.ActiveIdMouseButton = (ImS8)mouse_button_clicked;
                }
    ...
    if ((item_flags & ImGuiItemFlags_Disabled) == 0)
    {
        ...
        if (g.NavActivateDownId == id)
        {
            ...
            {
                // Set active id so it can be queried by user via IsItemActive(), equivalent of
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    ImDrawFlags rounding_corners =
        CalcRoundingFlagsForRectInRect(bb, window->Rect(), g.Style.WindowBorderSize);
                        ...
                        ImDrawFlags draw_rounding_flags)
{
    ...
    const float grab_h_pixels =
        (float)(int)ImClamp(scrollbar_size_v * ((float)size_visible_v / (float)win_size_v),
                            grab_h_minsize,
                            scrollbar_size_v);
    const float grab_h_norm = grab_h_pixels / scrollbar_size_v;

    // As a special thing, we allow scrollbar near the edge of a screen/viewport to be reachable
                        ...
                        const ImVec4 &tint_col)
{
    ...
    if (rounding > 0.0f)
        ...
        window->DrawList->AddImage(
            tex_ref, bb.Min + padding, bb.Max - padding, uv0, uv1, GetColorU32(tint_col));
    if (g.Style.ImageBorderSize > 0.0f)
        window->DrawList->AddRect(
            bb.Min, bb.Max, GetColorU32(ImGuiCol_Border), rounding, g.Style.ImageBorderSize);
}
                          ...
                          ImGuiButtonFlags flags)
{
    ...
    float image_rounding =
        ImMax(g.Style.FrameRounding - ImMax(padding.x, padding.y), g.Style.ImageRounding);
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        // Between 1.71 and 1.92.7, we maintained a hack where a 1.0f thin Separator() would not
        // impact layout. This was mostly chosen to allow backward compatibility with user's code
        // assuming zero-height when calculating height for layout (e.g. bottom alignment of a
        // status bar). In order to handle scaling we need to scale separator thickness and it would
        // not makes sense to have a disparity depending on height.
        ////float thickness_for_layout = (thickness == 1.0f) ? 0.0f : thickness; // FIXME:
        /// See 1.70/1.71 Separator() change: makes legacy 1-px separator not affect layout yet.
        /// Should change.
...
void ImGui::EndCombo()
{
    ...
    char name[16];
    ImFormatString(
        name, IM_COUNTOF(name), "##Combo_%02d", g.BeginComboDepth); // FIXME: Move those to helpers?
                                  ...
                                  void *p_data_when_empty)
{
    ...
    else
    {
        ...
        if (format[0] == '\0')
            format = type_info->ScanFmt; // Format doesn't want us to show the number currently, but
                                         // we still need to parse the resulting input
    }
                       ...
                       ImGuiSliderFlags flags)
{
    ...
    const ImGuiID id = window->GetID(label);
    const float w = CalcItemWidth();
    const ImU32 color_marker = (g.NextItemData.HasFlags & ImGuiNextItemDataFlags_HasColorMarker)
                         ...
                         ImGuiSliderFlags flags)
{
    ...
    if (temp_input_is_active)
    {
        ...
        const bool clamp_enabled =
            (flags & ImGuiSliderFlags_ClampOnInput) != 0; // Don't use TempInputIsClampEnabled()
        return TempInputScalar(frame_bb,
                               id,
                               label,
                               data_type,
                          ...
                          void *user_data)
{
    ...
    g.LastItemData.ItemFlags |=
        ImGuiItemFlags_AllowDuplicateId; // Using ImGuiInputTextFlags_MergedItem above will skip
    ...
    bool value_changed =
        InputTextEx(label,
                    ...
                    (int)buf_size,
                    bb.GetSize(),
                    flags | ImGuiInputTextFlags_TempInput | ImGuiInputTextFlags_AutoSelectAll,
                    callback,
                    user_data);
                        ...
                        ImGuiInputTextFlags flags)
{
    ...
    ImGuiContext &g = *GImGui;
    ImGuiStyle &style = g.Style;
    // IM_ASSERT((flags & ImGuiInputTextFlags_EnterReturnsTrue) == 0); // Not supported by
    // InputScalar(). Please open an issue if you this would be useful to you. Otherwise use
...
static int
STB_TEXTEDIT_MOVELINEEND_IMPL(ImGuiInputTextState *obj, ImStb::STB_TexteditState *state, int cursor)
{
    ...
    if (obj->WrapWidth > 0.0f)
    {
        ...
        while (p < text_end)
        {
            ...
            if (p_eol == p_cursor &&
                obj->LastMoveDirectionLR != ImGuiDir_Left) // If we are already on a visible
...
STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState *obj, int pos, const char *new_text, int new_text_len)
{
    ...
    if (!is_resizable && new_text_len > avail)
        new_text_len = (int)(ImTextFindValidUtf8CodepointEnd(
                                 new_text, new_text + new_text_len, new_text + avail) -
                             new_text); // Truncate to closest UTF-8 codepoint. Alternative: return
                                        // 0 to cancel insertion.
    if (new_text_len == 0)
        return 0;
                                     ...
                                     bool input_source_is_clipboard)
{
                  ...
                  (ImGuiInputTextFlags)ImGuiInputTextFlags_LocalizeDecimalPoint)))
    {
        ...
        // full-width characters that they may scratch their head as to why it works in numerical
        // fields vs in generic text fields it would require support in the font.
        if (flags & (ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsScientific |
    ...
    if (flags & ImGuiInputTextFlags_WordWrap)
    {
        for (s = buf; s < buf_end; s = (*s == '\n') ? s + 1 : s)
        {
            ...
            s = ImFontCalcWordWrapPositionEx(
                g.Font, g.FontSize, s, buf_end, wrap_width, ImDrawTextFlags_WrapKeepBlanks);
        }
    }
                                             ...
                                             int cursor_n)
{
    ...
    if (it > it_begin)
        if (it == it_end || *it != cursor_n ||
            (state != NULL && state->WrapWidth > 0.0f &&
             state->LastMoveDirectionLR == ImGuiDir_Right && cursor_ptr[-1] != '\n' &&
             cursor_ptr[-1] != 0))
            it--;
    ...
    ImVec2 offset;
    offset.x = InputTextCalcTextSize(
                   &g, line_start, cursor_ptr, buf_end, NULL, NULL, ImDrawTextFlags_WrapKeepBlanks)
                   .x;
                        ...
                        void *callback_user_data)
{
    ...
    if (is_multiline)
    {
        ImVec2 backup_pos = window->DC.CursorPos;
        ItemSize(total_bb, style.FramePadding.y);
        bool no_clip = (g.InputTextDeactivatedState.ID == id) || (g.ActiveId == id) ||
        ...
        // Prevent NavActivation from explicit Tabbing when our widget accepts Tab inputs: this
        // allows cycling through widgets without stopping.
    ...
    const bool input_requested_by_reactivate =
        (g.InputTextReactivateId == id); // for io.ConfigInputTextEnterKeepActive
    const bool input_requested_by_user =
        (user_clicked) || (g.ActiveId == 0 && (flags & ImGuiInputTextFlags_TempInput));
    const ImGuiID scrollbar_id =
        (is_multiline && state != NULL) ? GetWindowScrollbarID(draw_window, ImGuiAxis_Y) : 0;
    ...
    const bool init_changed_specs_readonly =
        (state != NULL && ((state->Flags ^ flags) &
                           ImGuiInputTextFlags_ReadOnly)); // state != NULL means its our state.
    const bool init_make_active = (input_requested_by_user || input_requested_by_nav ||
    ...
    else if ((init_make_active && g.ActiveId != id) || init_changed_specs_multiline ||
    ...
    if (state != NULL && state->ID == id)
    {
        ...
        // (FIXME-WORDWRAP: would be better to preserve same relative offset)
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        else if (is_enter || is_ctrl_enter || is_shift_enter || is_gamepad_validate)
        {
            ...
            if (!is_new_line)
            {
                ...
                {
                    // Queue reactivation, so that e.g. IsItemDeactivatedAfterEdit() will work.
                    // (#9001)
    ...
    if (g.ActiveId == id)
    {
                      ...
                      ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
        {
            ...
            if (event_flag)
            {
                ...
                // FIXME-OPT: Undo stack reconcile needs a backup of the data until we rework API,
                // see #7925
    ...
    // This is used when e.g. losing focus or tabbing out into another InputText() which may already
    // be using the temp buffer.
    ...
    // Write back result to user buffer. This can currently only happen when (g.ActiveId == id) or
    // when just deactivated.
    // - As soon as the InputText() is active, our stored in-widget value gets priority over any
    // underlying modification of the user buffer.
    // - Make sure we always reapply the live buffer back to the input/user buffer before clearing
    // ActiveId, even thought strictly speaking
    //   it was not modified on this frame. This allows the user to use InputText() without
    //   maintaining any user-side storage. (PS: if you use this property together with
    //   ImGuiInputTextFlags_CallbackResize, you are at the risk of recreating a temporary
    //    allocated/string object every frame. Which in the grand scheme of scheme is nothing, but
    //    isn't dear imgui vibe).
    if (apply_new_text != NULL)
    {
        ...
        if (is_resizable)
        {
            ...
            callback_data.EventActivated =
                (state != NULL && g.ActiveId == state->ID && g.ActiveIdIsJustActivated);
    ...
    if (is_multiline)
    {
        ...
        // FIXME-OPT: Could append to index when new value of line_visible_n1 becomes bigger, see
        // second call to CalcClipRectVisibleItemsY() below.
        bool will_scroll_y =
    ...
    const ImVec2 cursor_offset =
        render_cursor && state
            ? InputTextLineIndexGetPosOffset(
                  g, state, line_index, buf_display, buf_display_end, state->Stb->cursor)
            : ImVec2(0.0f, 0.0f);
    ImVec2 draw_scroll;
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (new_scroll_y != scroll_y)
        {
            const float scroll_max_y =
                ImMax((text_size_y + style.FramePadding.y * 2.0f) - inner_size.y, 0.0f);
        ...
        if (render_selection)
        {
            ...
            const float bg_offy_up =
                is_multiline ? 0.0f
                             : -1.0f; // FIXME-DPI: those offsets should be part of the style? they
    ...
    if (g.ActiveId != id && (flags & ImGuiInputTextFlags_ElideLeft) && !render_cursor &&
        !render_selection)
        draw_pos.x = ImMin(
            draw_pos.x, frame_bb.Max.x - CalcTextSize(buf_display, NULL).x - style.FramePadding.x);
    ...
    if (render_cursor)
    {
        ...
        if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
            draw_window->DrawList->AddLineV(
                ...
                1.0f * (float)(int)style._MainScale); // FIXME-DPI: Cursor thickness (#7031)

        // Notify OS of text input position for advanced IME (-1 x offset so that Windows IME can
        // cover our cursor. Bit of an extra nicety.) This is required for some backends (SDL3) to
        // start emitting character/text inputs. As per #6341, make sure we don't set that on the
        // deactivating frame.
...
bool ImGui::ColorEdit4(const char *label, float col[4], ImGuiColorEditFlags flags)
{
        ...
        (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        const ImGuiSliderFlags drag_flags =
            draw_color_marker ? ImGuiSliderFlags_ColorMarkers : ImGuiSliderFlags_None;
        ...
        for (int n = 0; n < components; n++)
        {
            ...
            else
            {
                value_changed |= DragInt(
                    ids[n], &i[n], 1.0f, 0, hdr ? 0 : 255, fmt_table_int[fmt_idx][n], drag_flags);
            }
            if (!(flags & ImGuiColorEditFlags_NoOptions))
    ...
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            ...
            window->DrawList->AddRect(
                ...
                rounding,
                1.0f * (float)(int)g.Style._MainScale); // Color buttons are often in need of some
                                                        // sort of border // FIXME-DPI
    }
...
// The reason those two functions are not yet in public API is because I would like to design a more
// feature-full and generic API for this. They are otherwise function (cc: #3823, #9251, #7553,
// #6754, #5423, #2958, #2079, #1947, #1131, #722)
                             ...
                             const char *label_end)
{
    ...
    const float text_offset_y =
        use_frame_padding ? ImMax(style.FramePadding.y, window->DC.CurrLineTextBaseOffset)
                           ...
                           ImGuiMultiSelectFlags ms_flags)
{
    ...
    // They are derived from positions relative to scrolling space, so "previous" rectangle is
    // reprojected for current frame coordinates.
    ...
    // bs->StartPosRel.x, bs->StartPosRel.y, bs->EndPosRel.x, bs->EndPosRel.y,
    // WindowPosAbsToRel(window, g.IO.MousePos).x, WindowPosAbsToRel(window, g.IO.MousePos).y);

    // Box-select 2D mode detects change of the rectangle.
    // Storing unclip rects which will be tested by widgets supporting box-select. Always update
    // rectangles when active (even if we don't use them). To facilitate understanding this: enable
    // IMGUI_DEBUG_BOXSELECT and visualize all geometry.
    if (ms_flags & (ImGuiMultiSelectFlags_BoxSelect1d | ImGuiMultiSelectFlags_BoxSelect2d))
    {
        ...
        {
            ImVec2 d_min = (side == 0)
                               ? ImMin(bs->BoxSelectRectCurr.Min, bs->BoxSelectRectPrev.Min)
                               : ImMin(bs->BoxSelectRectCurr.Max, bs->BoxSelectRectPrev.Max);
            ImVec2 d_max = (side == 0)
                               ? ImMax(bs->BoxSelectRectCurr.Min, bs->BoxSelectRectPrev.Min)
        ...
        // FIXME-OPT: When UnclipRect.Area() is much larger than the sum of UnclipRects[0]/[1]
        // Areas, widgets should ideally first use UnclipRect as a first coarse cull layer + the
        // individual ones as a second validation.
...
#ifdef IMGUI_DEBUG_BOXSELECT
    // GetForegroundDrawList()->AddRect(scope_rect.Min, scope_rect.Max, IM_COL32(0, 255, 0, 200),
    // 0.0f, 0, 4.0f); GetForegroundDrawList()->AddRect(bs->BoxSelectRectPrev.Min,
    ...
    if (ms_flags & (ImGuiMultiSelectFlags_BoxSelect1d | ImGuiMultiSelectFlags_BoxSelect2d))
    {
        for (ImRect &unclip_r : bs->UnclipRects)
            if (!unclip_r.IsInverted())
                GetForegroundDrawList()->AddRect(unclip_r.Min,
                                                 unclip_r.Max,
...
static ImRect CalcScopeRect(ImGuiMultiSelectTempData *ms, ImGuiWindow *window)
{
    if (ms->Flags & ImGuiMultiSelectFlags_ScopeRect)
    {
        ...
        // This probably doesn't work inside a table as there are ample ambiguities related to exact
        // time of calling BeginMultiSelect()/EndMultiSelect().
...
ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size, int items_count)
{
    ...
    if (flags & ImGuiMultiSelectFlags_ScopeRect)
        window->DC.CursorMaxPos =
            ms->ScopeRectMin; // CalcScopeRect() for ImGuiMultiSelectFlags_ScopeRect will measure in
                              // EndMultiSelect().
            ...
            (ms->RangeSrcPassedBy == false &&
             ms->IO.RangeSrcItem !=
                 ImGuiSelectionUserData_Invalid)) // Can't read storage->RangeSrcItem here -> we
                                                  // want the state at beginning of the scope (see
...
void ImGui::SetNextItemSelectionUserData(ImGuiSelectionUserData selection_user_data)
{
    ...
    if (ImGuiMultiSelectTempData *ms = g.CurrentMultiSelect)
    {
        ...
        // ms->PrevSubmittedItem = ms->CurrSubmittedItem; // Can't rely on previous
        // g.NextItemData.SelectionUserData because NextItemData is not restored on nested
        // multi-select. ms->CurrSubmittedItem = selection_user_data;
    }
    else
    {
...
void ImGui::MultiSelectItemFooter(ImGuiID id, bool *p_selected, bool *p_pressed)
{
            ...
            if (!window->DC.NavIsScrollPushableX) // FIXME: Rename to be more generic.
                if (ImGuiTable *table = g.CurrentTable)
                    if (table->CurrentColumn != -1)
                    {
                        ...
                        // However we account for ClipRect being larger than current column (e.g.
                        // when using SpanAllColumns) A more generic version would be nice, but
                        // window->WorkRect.Min/Max exclude CellPadding. (#7994, #9383)
                        ImGuiTableColumn *column = &table->Columns[table->CurrentColumn];
                        float clip_min_x =
                            (g.LastItemData.ItemFlags & ImGuiItemStatusFlags_HasClipRect)
                        ...
                        if (clip_min_x != clip_max_x) // When zero sized we expect that bounds have
                                                      // been clamped and thus are unreliable
                        {
                            item_rect.Min.x =
                                ImMax(item_rect.Min.x, ImMin(column->MinX, clip_min_x));
                            item_rect.Max.x =
                                ImMin(item_rect.Max.x, ImMax(column->MaxX, clip_max_x));
                        }
                        else
                        {
                        ...
                        // GetForegroundDrawList()->AddRect(item_rect.Min, item_rect.Max,
...
#ifdef IMGUI_DEBUG_BOXSELECT
                    GetForegroundDrawList()->AddRectFilled(g.LastItemData.Rect.Min,
    ...
    if (hovered && IsMouseClicked(1) &&
        (flags &
         (ImGuiMultiSelectFlags_NoAutoSelect | ImGuiMultiSelectFlags_NoSelectOnRightClick)) == 0)
    {
                                   ...
                                   ImGuiSelectionUserData last_item)
{
    ...
    // single-item ranges)
    // FIXME-OPT: Disabled on 2026/04/09 as this would break with any form of coarse clipping that
    // we don't know about (e.g. TableNextColumn() return value). The low-hanging fruit would be to
    // know that ImGuiSelectionUserData are sequential indices, in which case we can trivially
    // compare PrevSubmittedItem + RangeDir == FirstItem. User can always perform this merge if
    // required.
#if 0
...
// Plot Histogram (the data provided _is_ histogram data. it doesn't compute the histogram of your
// data)
...
void ImGui::EndMainMenuBar()
{
    ...
    IM_ASSERT_USER_ERROR_RET(
        ...
        "Calling EndMainMenuBar() not from a menu-bar!"); // Not technically testing that it is the
                                                          // main menu bar
...
bool ImGui::BeginMenuEx(const char *label, const char *icon, bool enabled)
{
    ...
    else
    {
        ...
        float min_w =
            offsets->DeclColumns(icon_w, label_size.x, 0.0f, checkmark_w); // Feedback to next frame
    ...
    // Once dragged, release ActiveId + key ownership. This is to allow the idiom of mouse down a
    // menu, dragging elsewhere, up on some other MenuItem(). (#8233, #9394) Could move logic into
    // lower-level ImGuiButtonFlags_AutoReleaseActiveId + ImGuiButtonFlags_AutoReleaseKeyOwner?
    // Easier once we get rid of the Selectable() middle-man here.
    if (g.ActiveId == id && g.HoveredId != id && g.ActiveIdSource == ImGuiInputSource_Mouse &&
        IsMouseDragging(0))
    {
        ClearActiveID();
...
static void ImGui::TabBarLayout(ImGuiTabBar *tab_bar)
{
    ...
    sections[0].Spacing =
        sections[0].TabCount > 0 && (sections[1].TabCount + sections[2].TabCount) > 0 ? tab_spacing
                                                                                      : 0.0f;
...
bool ImGui::BeginTabItem(const char *label, bool *p_open, ImGuiTabItemFlags flags)
{
    ...
    IM_ASSERT_USER_ERROR_RETV(
        tab_bar != NULL, false, "Needs to be called between BeginTabBar() and EndTabBar()!");
                      ...
                      ImGuiWindow *docked_window)
{
    ...
    if (want_clip_rect)
        PushClipRect(
            ImVec2(ImClamp(bb.Min.x, tab_bar->ScrollingRectMinX, tab_bar->ScrollingRectMaxX),
                   bb.Min.y - 1),
    ...
    ImGuiButtonFlags button_flags =
        ((ImGuiButtonFlags)(is_tab_button ? ImGuiButtonFlags_PressedOnClickRelease
                                          : ImGuiButtonFlags_PressedOnClick) |
