...
// - Add '#define IMGUI_DEFINE_MATH_OPERATORS' before including imgui.h (or in imconfig.h) to access
// courtesy maths operators for ImVec2 and ImVec4.
...
// Helper Macros
// (note: compiling with NDEBUG will usually strip out assert() to nothing, which is NOT recommended
...
// This allowed backends requiring 64-bit worth of data to build on 32-bit architectures. Use
...
// - If your backend is using ImTextureID to store an index/offset and you need 0 to be valid, You
// can add '#define ImTextureID_Invalid ((ImTextureID)-1)' in your imconfig.h file.
// - From 2026/03/12 to 2026/03/19 we experimented with changing to default to -1, but I worried it
// would cause too many issues in third-party code so it was reverted.
...
// - When a texture is created by user code (e.g. custom images), we directly store the low-level
// ImTextureID.
...
struct ImTextureRef
{
    ...
    inline ImTextureID
    GetTexID() const; // == (_TexData ? _TexData->TexID : _TexID) // Implemented below in the file.
...
IMGUI_API bool IsWindowHovered(
    ImGuiHoveredFlags flags =
        0); // is current window hovered and hoverable (e.g. not blocked by a popup/modal)? See
            ...
            // use this function! Use the 'io.WantCaptureMouse' boolean for that! Refer to FAQ entry
...
IMGUI_API bool
SmallButton(const char *label); // button with (FramePadding.y == 0) to easily embed within text
...
// - If you want to use InputText() with std::string or any custom dynamic string type, use the
// wrapper in misc/cpp/imgui_stdlib.h/.cpp!
...
IMGUI_API bool TreeNode(const char *str_id, const char *fmt, ...)
    IM_FMTARGS(2); // helper variation to easily decorrelate the id from the displayed string. Read
...
//  - IMPORTANT: If you ever used the left mouse button with BeginPopupContextXXX() helpers
//  before 1.92.6:
//    - Before this version, OpenPopupOnItemClick(), BeginPopupContextItem(),
//    BeginPopupContextWindow(), BeginPopupContextVoid() had 'a ImGuiPopupFlags popup_flags = 1'
//    default value in their function signature.
//    - Before: Explicitly passing a literal 0 meant ImGuiPopupFlags_MouseButtonLeft. The default =
//    1 meant ImGuiPopupFlags_MouseButtonRight.
//    - After: The default = 0 means ImGuiPopupFlags_MouseButtonRight. Explicitly passing a literal
//    1 also means ImGuiPopupFlags_MouseButtonRight (if legacy behavior are enabled) or will assert
//    (if legacy behavior are disabled).
//    - TL;DR: if you don't want to use right mouse button for popups, always specify it explicitly
//    using a named ImGuiPopupFlags_MouseButtonXXXX value.
//    - Read "API BREAKING CHANGES" 2026/01/07 (1.92.6) entry in imgui.cpp or GitHub topic #9157 for
//    all details.
IMGUI_API bool BeginPopupContextItem(
    const char *str_id = NULL,
    ImGuiPopupFlags popup_flags =
        0); // open+begin popup when clicked on last item. Use str_id==NULL to associate the popup
...
// - Use TableSetupScrollFreeze() to lock columns/rows so they stay visible when scrolled. When
// freezing columns you would usually also use ImGuiTableColumnFlags_NoHide on them.
...
// - Tooltips windows are automatically opted out of disabling. Note that IsItemHovered() by default
// returns false on disabled items, unless using ImGuiHoveredFlags_AllowWhenDisabled.
...
IMGUI_API void
SetNavCursorVisible(bool visible); // alter visibility of keyboard/gamepad cursor. by default: show
                                   // when using an arrow key, hide when clicking with mouse.

// Overlapping mode
IMGUI_API void
SetNextItemAllowOverlap(); // allow next item to be overlapped by a subsequent item. Typically
                           // useful with InvisibleButton(), Selectable(), TreeNode() covering an
                           // area where subsequent items may need to be added. Note that both
                           // Selectable() and TreeNode() have dedicated flags doing this.
...
// - Consider using the Shortcut() function instead of IsKeyPressed()/IsKeyChordPressed()!
// Shortcut() is easier to use and better featured (can do focus routing check).
...
// - (legacy: before v1.87 (2022-02), we used ImGuiKey < 512 values to carry native/user indices as
// defined by each backends. This was obsoleted in 1.87 (2022-02) and completely removed in 1.91.5
// (2024-11). See https://github.com/ocornut/imgui/issues/4921)
IMGUI_API bool IsKeyDown(ImGuiKey key); // is key being held.
IMGUI_API bool
IsKeyPressed(ImGuiKey key, bool repeat = true); // was key pressed (went from !Down to Down)? Repeat
...
// - Flags: Default route use ImGuiInputFlags_RouteFocused, but see ImGuiInputFlags_RouteGlobal and
// other options in ImGuiInputFlags_!
...
//   IsKeyChordPressed()
//     -> the function has (desirable) side-effects as it can prevents another call from getting the
//     route.
...
//   e.g. `Button(...); if (SetItemKeyOwner(ImGuiKey_MouseWheelY)) { ... }` to make
...
// to them.
// - The return value of SetItemKeyOwner() says if ownership has been requested for the item, which
// is a shortcut to calling yet non-public TestKeyOwner() function.
...
IMGUI_API bool
SetItemKeyOwner(ImGuiKey key); // Set key owner to last item ID if it is hovered or active. Return
                               // true when ownership has been set. Roughly equivalent to 'if
                               // (TestKeyOwner(key, GetItemID()) && (IsItemHovered() ||
...
IMGUI_API bool
IsMouseHoveringRect(const ImVec2 &r_min,
                    const ImVec2 &r_max,
...
// - Read https://github.com/ocornut/imgui/wiki/Debug-Tools for a description of all available debug
// tools.
IMGUI_API void DebugTextEncoding(const char *text);
IMGUI_API void DebugFlashStyleColor(ImGuiCol idx);
IMGUI_API void DebugStartItemPicker();
IMGUI_API bool
DebugCheckVersionAndDataLayout(const char *version_str,
                               size_t sz_io,
                               size_t sz_style,
                               size_t sz_vec2,
                               size_t sz_vec4,
...
enum ImGuiWindowFlags_
{
    ...
    // ImGuiWindowFlags_NavFlattened           = 1 << 29,  // Obsoleted in 1.90.9: moved to
    // ImGuiChildFlags. BeginChild(name, size, 0, ImGuiWindowFlags_NavFlattened)           -->
    // BeginChild(name, size, ImGuiChildFlags_NavFlattened, 0)
    // ImGuiWindowFlags_AlwaysUseWindowPadding = 1 << 30,  // Obsoleted in 1.90.0: moved to
    // ImGuiChildFlags. BeginChild(name, size, 0, ImGuiWindowFlags_AlwaysUseWindowPadding) -->
...
enum ImGuiChildFlags_
{
    ...
    ImGuiChildFlags_AlwaysAutoResize =
        1
        << 6, // Combined with AutoResizeX/AutoResizeY. Always measure size even when child is
    ...
    // Obsolete names
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    // ImGuiChildFlags_Border                = ImGuiChildFlags_Borders,  // Renamed in 1.91.1
    // (August 2024) for consistency.
#endif
};
...
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_Disabled =
        1 << 6, // false    // [Internal] Disable interactions. DOES NOT affect visuals. This is
                // used by BeginDisabled()/EndDisabled() and only provided here so you can read back
                // via GetItemFlags().
};
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CtrlEnterForNewLine =
        1 << 8, // In multi-line mode: validate with Enter, add new line with Ctrl+Enter (default is
                // opposite: validate with Ctrl+Enter, add line with Enter). Note that Shift+Enter
                // always enter a new line either way.
    ...
    // - Not well tested yet. Please report any incorrect cursor movement, selection behavior etc.
    // bug to https://github.com/ocornut/imgui/issues/3237.
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_AllowOverlap =
        1 << 2, // Hit testing will allow subsequent widgets to overlap this one. Require previous
                // frame HoveredId to match before being usable. Shortcut to calling
    ...
    ImGuiTreeNodeFlags_Leaf =
        1 << 8, // No collapsing, no arrow (use as a convenience for leaf nodes). Note: will always
                // open a tree/id scope and return true. If you never use that scope, add
                // ImGuiTreeNodeFlags_NoTreePushOnOpen.
    ...
    ImGuiTreeNodeFlags_NavLeftJumpsToParent =
        1 << 17, // Nav: left arrow moves back to parent. This is processed in TreePop() when
                 // there's an unfulfilled Left nav request remaining.
...
// - IMPORTANT: If you ever used the left mouse button with BeginPopupContextXXX() helpers
// before 1.92.6: Read "API BREAKING CHANGES" 2026/01/07 (1.92.6) entry in imgui.cpp or GitHub topic
// #9157.
...
enum ImGuiPopupFlags_
{
    ...
    ImGuiPopupFlags_MouseButtonRight = 2 << 2, // For BeginPopupContext*(): open on Right Mouse
    ...
    ImGuiPopupFlags_InvalidMask_ = 0x03, // [Internal] Reserve legacy bits 0-1 to detect incorrectly
...
enum ImGuiSelectableFlags_
{
    ...
    ImGuiSelectableFlags_AllowOverlap =
        1 << 4, // Hit testing will allow subsequent widgets to overlap this one. Require previous
                // frame HoveredId to match before being usable. Shortcut to calling
                // SetNextItemAllowOverlap().
...
enum ImGuiTabBarFlags_
{
    ...
    ImGuiTabBarFlags_FittingPolicyMixed =
        1 << 7, // Shrink down tabs when they don't fit, until width is style.TabMinWidthShrink,
                // then enable scrolling. Setting TabMinWidthShrink to FLT_MAX makes this behave
...
enum ImGuiHoveredFlags_
{
    ...
    //   e.g. 'HoverFlagsForTooltipMouse' defaults to 'ImGuiHoveredFlags_Stationary |
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_AcceptDrawAsHovered =
        1 << 13, // Accepting item will render as if hovered. Useful for e.g. a Button() used as a
                 // drop target.
...
// "Keys" related to physical keys and are not the same concept as input "Characters", the latter
// are submitted via io.AddInputCharacter(). The keyboard key enum values are named after the keys
// on a standard US keyboard, and on other keyboard types the keys reported may not match the
// keycaps.
enum ImGuiKey : int
{
    // Keyboard
    ...
    ImGuiKey_GamepadFaceLeft,  // X           | Y       | Square   | Toggle Menu. Hold for Windowing
    ...
    ImGuiKey_GamepadFaceDown,  // A           | B       | Cross    | Activate / Open / Toggle. Hold
                               // for 0.60f to Activate in Text Input mode (e.g. wired to an
                               // on-screen keyboard).
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    ImGuiKey_COUNT = ImGuiKey_NamedKey_END, // Obsoleted in 1.91.5 because it was misleading (since
...
// Note that nowadays most of our configuration options are in other ImGuiIO fields, e.g.
...
enum ImGuiConfigFlags_
{
    ...
    ImGuiConfigFlags_NavEnableKeyboard =
        1 << 0, // Master keyboard navigation enable flag. Enable full Tabbing + directional arrows
                // + Space/Enter to activate. Note: some features such as basic Tabbing and CtrL+Tab
                // are enabled by regardless of this flag (and may be disabled via other means, see
                // #4828, #9218).
    ...
    // [Unused] User storage (to allow your backend/engine to communicate to code that may be shared
...
enum ImGuiButtonFlags_
{
    ...
    ImGuiButtonFlags_AllowOverlap =
        1 << 12, // Hit testing will allow subsequent widgets to overlap this one. Require previous
                 // frame HoveredId to match before being usable. Shortcut to calling
                 // SetNextItemAllowOverlap().
};
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_NoDragDrop =
        1 << 9, //              // ColorEdit: disable drag and drop target/source. ColorButton:
    ...
    ImGuiColorEditFlags_NoColorMarkers =
        1 << 11, //              // ColorEdit: disable rendering R/G/B/A color marker. May also be
    ...
    ImGuiColorEditFlags_AlphaOpaque =
        1 << 12, //              // ColorEdit, ColorPicker, ColorButton: disable alpha in the
    ...
    ImGuiColorEditFlags_AlphaNoBg =
        1 << 13, //              // ColorEdit, ColorPicker, ColorButton: disable rendering a
                 //              checkerboard background behind transparent color.
    ImGuiColorEditFlags_AlphaPreviewHalf =
        1 << 14, //              // ColorEdit, ColorPicker, ColorButton: display half opaque / half
                 //              transparent preview.

    // User Options (right-click on widget to change some of them).
    ImGuiColorEditFlags_AlphaBar = 1 << 18, //              // ColorEdit, ColorPicker: show vertical
...
enum ImGuiSliderFlags_
{
    ...
    ImGuiSliderFlags_InvalidMask_ =
        0x7000000F, // [Internal] We treat using those bits as being potentially a 'float power'
                    // argument from legacy API (obsoleted 2020-08) that has got miscast to this
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_Reorderable =
        1 << 1, // Enable reordering columns in header row. (Need calling TableSetupColumn() +
                // TableHeadersRow() to display headers, or using ImGuiTableFlags_ContextMenuInBody
    ...
    ImGuiTableFlags_NoSavedSettings = 1 << 4, // Disable persisting columns order, width, visibility
                                              // and sort settings in the .ini file.
    ImGuiTableFlags_ContextMenuInBody =
        1 << 5, // Right-click on columns body/contents will also display table context menu. By
    ...
    ImGuiTableFlags_NoPadInnerX =
        1 << 23, // Disable inner padding between columns (double inner padding if BordersOuterV is
                 // on, single inner padding if BordersOuterV is off).
    // Scrolling
...
enum ImGuiTableColumnFlags_
{
    ...
    ImGuiTableColumnFlags_NoHeaderLabel =
        1 << 12, // TableHeadersRow() will submit an empty label for this column. Convenient for
                 ...
                 // You may append into this cell by calling TableSetColumnIndex() right after the
...
struct ImGuiStyle
{
    ...
    float FontScaleMain; // Main global scale factor. May be set by application once, or exposed to
                         // end-user.
    float FontScaleDpi;  // Additional global scale factor from viewport/monitor contents scale. In
                         // docking branch: when io.ConfigDpiScaleFonts is enabled, this is
    ...
    float DragDropTargetRounding;   // Radius of the drag and drop target frame. When <0.0f: use
                                    // FrameRounding.
    float DragDropTargetBorderSize; // Thickness of the drag and drop target border.
    float DragDropTargetPadding; // Size to expand the drag and drop target from actual target item
                                 // size.
    float ColorMarkerSize; // Size of R/G/B/A color markers for ColorEdit4() and for Drags/Sliders
                           // when using ImGuiSliderFlags_ColorMarkers.
    ...
    // [Internal]
    float _MainScale; // FIXME-WIP: Reference scale, as applied by ScaleAllSizes(). PLEASE DO NOT
                      // USE THIS FOR NOW.
    ...
    IMGUI_API void
    ScaleAllSizes(float scale_factor); // Scale all spacing/padding/thickness values. Do not scale
                                       // fonts. See comments in definition. Consider not calling
                                       // this if your initial scale factor if <1.0.
...
struct ImGuiIO
{
    ...
    bool ConfigInputTextEnterKeepActive; // = false          // [BETA] Pressing Enter will
                                         // reactivate item and select all text (single-line only).
    ...
    //    - Recovery after a programming error (native code or scripting language - the latter tends
    ...
    IMGUI_API void
    SetKeyEventNativeData(ImGuiKey key,
                          int native_keycode,
                          int native_scancode,
    ...
    // void ClearInputCharacters() { InputQueueCharacters.resize(0); } // [Obsoleted in 1.89.8]
    // Clear the current frame text input buffer. Now included within ClearInputKeys(). Removed this
    // as it is ambiguous/misleading and generally incorrect to use with the existence of a
...
struct ImGuiListClipper
{
    ...
    int UserIndex;    // Helper storage for user convenience/code. Optional, and otherwise unused if
                      // you don't use it.
    ...
    // item_end); } // [renamed in 1.89.6] inline ImGuiListClipper(int items_count, float
    // items_height = -1.0f) { memset((void*)this, 0, sizeof(*this)); ItemsCount = -1;
...
enum ImGuiMultiSelectFlags_
{
    ...
    ImGuiMultiSelectFlags_SingleSelect =
        1 << 0, // Disable selecting more than one item. This is available to allow single-selection
                // code to share same code/logic if desired. It essentially disables the main
                // purpose of BeginMultiSelect() tho!
    ...
    ImGuiMultiSelectFlags_BoxSelect2d =
        1
        << 7, // Enable box-selection with varying width or varying x pos items support (e.g.
    ...
    ImGuiMultiSelectFlags_SelectOnAuto =
        1 << 13, // Apply selection on mouse down when clicking on unselected item, on mouse up when
                 // clicking on selected item. (Default)
    ImGuiMultiSelectFlags_SelectOnClickAlways =
        1 << 14, // Apply selection on mouse down when clicking on any items. Prevents Drag and Drop
                 // from being used on multiple-selection, but allows e.g. BoxSelect to always
                 // reselect even when clicking inside an existing selection. (Excel style behavior)
    ImGuiMultiSelectFlags_SelectOnClickRelease =
        1 << 15, // Apply selection on mouse release when clicking an unselected item. Allow
    ...
    ImGuiMultiSelectFlags_NoSelectOnRightClick =
        1 << 17, // Disable default right-click processing, which selects item on mouse down, and is
                 // designed for context-menus.
...
struct ImDrawCmd
{
    ...
    inline ImTextureID
    GetTexID() const; // == (TexRef._TexData ? TexRef._TexData->TexID : TexRef._TexID)
};
...
struct ImDrawList
{
    ...
    IMGUI_API void
    AddRectFilled(const ImVec2 &p_min,
                  const ImVec2 &p_max,
                  ImU32 col,
                  float rounding = 0.0f,
                  ImDrawFlags flags = 0); // a: upper-left, b: lower-right (== upper-left + size)
    ...
    IMGUI_API void AddPolyline(
        const ImVec2 *points, int num_points, ImU32 col, float thickness, ImDrawFlags flags = 0);
    ...
    // - Use special GetPlatformIO().DrawCallback_ResetRenderState callback to instruct backend to
    // reset its render state to the default.
    // - See other standard callbacks in GetPlatformIO(), which may or not be supported by your
    // backend.
    ...
    IMGUI_API void
    AddCallback(ImDrawCallback callback, void *userdata = NULL, size_t userdata_size = 0);
    ...
    IMGUI_API ImDrawList *
    CloneOutput() const; // Create a clone of the CmdBuffer/IdxBuffer/VtxBuffer. For multi-threaded
                         // rendering, consider using `imgui_threaded_rendering` from
    ...
    inline void
    PathStroke(ImU32 col, ImDrawFlags flags /*= 0*/, float thickness /*= 1.0f*/) = delete;
#endif
...
struct ImTextureData
{
    ...
    // Functions
    // - If GetPixels() functions asserts while being called by your render loop, it could be caused
    // by calling ImFontAtlas::Clear() instead of ClearFonts()?
    ImTextureData()
    {
        memset((void *)this, 0, sizeof(*this));
...
struct ImFontConfig
{
    ...
    bool FontDataOwnedByAtlas; // true     // TTF/OTF data ownership taken by the owner ImFontAtlas
                               // (will delete memory itself). SINCE 1.92, THE DATA NEEDS TO PERSIST
    ...
    bool PixelSnapH;  // false    // Align every glyph AdvanceX to pixel boundaries. Prevents
                      // fractional font size from working correctly! Useful e.g. if you are merging
                      // a non-pixel aligned font with the default font. If enabled, OversampleH/V
                      // will default to 1.
    ...
    float SizePixels; //          // Output size in pixels for rasterizer (more or less maps to the
    ...
    // PLEASE REPORT IF YOU WERE USING THIS). Extra spacing (in pixels) between glyphs when
    // rendered: essentially add to glyph->AdvanceX. Only X axis is supported for now.
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    bool PixelSnapV; // true    // [Obsoleted in 1.91.6] Align Scaled GlyphOffset.y to pixel
                     // boundaries.
#endif
...
struct ImFontAtlas
{
    ...
    IMGUI_API ImFont *
    AddFontDefaultVector(const ImFontConfig *font_cfg =
    ...
    IMGUI_API void ClearFonts(); // Clear input+output font data/glyphs. You can call this mid-frame
                                 // if you load new fonts afterwards!
    ...
    // - Since 1.92.0, packing is done immediately in the function call (previously packing was done
    ...
    IMGUI_API void
    RemoveCustomRect(ImFontAtlasRectId id); // Unregister a rectangle. Existing pixels will stay in
    ...
    inline void
    CalcCustomRectUV(const ImFontAtlasRect *r, ImVec2 *out_uv_min, ImVec2 *out_uv_max) const
    {
        *out_uv_min = r->uv0;
        *out_uv_max = r->uv1;
    } // OBSOLETED in 1.92.0
    IMGUI_API ImFontAtlasRectId AddCustomRectFontGlyph(
        ...
        float advance_x,
        const ImVec2 &offset =
            ImVec2(0, 0)); // OBSOLETED in 1.92.0: Use custom ImFontLoader in ImFontConfig
...
#endif
    // unsigned int                      FontBuilderFlags;        // OBSOLETED in 1.92.0: Renamed to
    // FontLoaderFlags. int                               TexDesiredWidth;         // OBSOLETED
    // in 1.92.0: Force texture width before calling Build(). Must be a power-of-two. If have many
    ...
    // OBSOLETED in 1.92.0 typedef ImFontAtlasCustomRect     CustomRect;              // OBSOLETED
...
struct ImFontBaked
{
    ...
    FindGlyph(ImWchar c); // Return U+FFFD glyph if requested glyph doesn't exists.
...
// manually.
// - Since 1.92.0 a font may be rendered as any size! Therefore a font doesn't have one specific
// size.
...
struct ImFont
{
    ...
    ImWchar EllipsisChar; // 2-4   // out // Character used for ellipsis rendering ('...'). If you
                          // ever want to temporarily swap this for an alternative/dummy char, make
                          // sure to clear EllipsisAutoBake.
    ImWchar FallbackChar; // 2-4   // out // Character used if a glyph isn't found (U+FFFD, '?')
    ImU8 Used8kPagesMap[(IM_UNICODE_CODEPOINT_MAX + 1) / 8192 /
                        8]; // 1 bytes if ImWchar=ImWchar16, 17 bytes if ImWchar==ImWchar32. Store
                            // 1-bit for each block of 8K codepoints that has one active glyph. This
                            // is mainly used to facilitate iterations across all used codepoints.
    bool EllipsisAutoBake;  // 1     //     // Mark when the "..." glyph (== EllipsisChar) needs to
                            // be generated by combining multiple '.'.
    ImGuiStorage
    ...
    IMGUI_API ImFontBaked *
    GetFontBaked(float font_size, float density = -1.0f); // Get or create baked data for given size
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    inline const char *
    CalcWordWrapPositionA(float scale, const char *text, const char *text_end, float wrap_width)
    {
...
{
    // If you are getting this assert with ImTextureID_Invalid == 0 and your ImTextureID is used to
    // store an index or an offset:
    // - You can add '#define ImTextureID_Invalid ((ImTextureID)-1)' in your imconfig.h file.
    // If you are getting this assert with a renderer backend with support for
    // ImGuiBackendFlags_RendererHasTextures (1.92+):
    ...
    if (TexRef._TexData != NULL)
        IM_ASSERT(tex_id != ImTextureID_Invalid &&
                  "ImDrawCmd is referring to ImTextureData that wasn't uploaded to graphics "
                  "system. Backend must call ImTextureData::SetTexID() after handling "
                  "ImTextureStatus_WantCreate request!");
    return tex_id;
}
...
struct ImGuiPlatformIO
{
    ...
    // (default to use ShellExecuteW() on Windows, system() on Linux/Mac. expected to return false
    // on failure, but some platforms may always return true)
    ...
    // ImDrawCallback  DrawCallback_SetSamplerCustom;    // Request backend to set texture sampling
    ...
    IMGUI_API void ClearPlatformHandlers(); // Clear all Platform_XXX fields. Typically called on
                                            // Platform Backend shutdown.
    IMGUI_API void ClearRendererHandlers(); // Clear all Renderer_XXX fields. Typically called on
...
// reported in IDE) OBSOLETED in 1.90.0 (from September 2023)
// IMGUI_API bool      Combo(const char* label, int* current_item, bool (*old_callback)(void*
// user_data, int idx, const char** out_text), void* user_data, int items_count, int
// popup_max_height_in_items = -1); // Getter signature changed. See 2023/09/15 and 2026/02/27
// commits. IMGUI_API bool      ListBox(const char* label, int* current_item, bool
// (*old_callback)(void* user_data, int idx, const char** out_text), void* user_data, int
// items_count, int height_in_items = -1);         // Getter signature changed. See 2023/09/15 and
// 2026/02/27 commits. inline bool         BeginChild(const char* str_id, const ImVec2& size_arg,
// bool borders, ImGuiWindowFlags window_flags) { return BeginChild(str_id, size_arg, borders ?
...
// ImGuiChildFlags_Borders : ImGuiChildFlags_None, window_flags);     } // Unnecessary as true ==
// ImGuiChildFlags_Borders inline bool         BeginChildFrame(ImGuiID id, const ImVec2& size,
// ImGuiWindowFlags flags = 0) { return BeginChild(id, size, ImGuiChildFlags_FrameStyle, flags); }
...
// commented out ahead of obsolescence schedule to reduce confusion and because they were not meant
...
// #define IM_OFFSETOF(_TYPE,_MEMBER)  offsetof(_TYPE, _MEMBER)  // OBSOLETED IN 1.90 (now using
