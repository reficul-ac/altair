...
typedef int ImGuiWindowBgClickFlags; // -> enum ImGuiWindowBgClickFlags_ // Flags: for overriding
                                     // behavior of clicking on window background/void.
...
// Helper: ImStableVector<>
// Allocating chunks of BLOCKSIZE items. Objects pointers are never invalidated when growing, only
...
enum ImGuiInputTextFlagsPrivate_
{
    ...
    ImGuiInputTextFlags_TempInput = 1
...
enum ImGuiButtonFlagsPrivate_
{
    ...
    ImGuiButtonFlags_PressedOnRelease =
        1 << 7, // return true on release (default requires click+release). Prior to 2026/03/20 this
                // implied ImGuiButtonFlags_NoHoldingActiveId but they are separate now.
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    const char
        *TextSrc; // == TextA.Data unless read-only, in which case == buf passed to InputText(). For
                  // _ReadOnly fields, pointer will be null outside the InputText() call.
...
struct ImGuiNextItemData
{
    ...
    ImU32 ColorMarker;             // Set by SetNextItemColorMarker(). Not exposed yet, supported by
                                   // DragScalar,SliderScalar and for ImGuiSliderFlags_ColorMarkers.
...
struct IMGUI_API ImGuiMultiSelectTempData
{
    ...
    ImVec2 ScopeRectMin;
    ImVec2 BackupCursorMaxPos;
    // ImGuiSelectionUserData CurrSubmittedItem; // Copy of last submitted item data, used to merge
    // output ranges. ImGuiSelectionUserData PrevSubmittedItem; // Copy of previous submitted item
    // data, used to merge output ranges.
    ImGuiID BoxSelectId;
...
{
    float BgFgDrawListsLastTimeActive[2]; // Last frame number the background (0) and foreground (1)
...
struct ImGuiDebugItemPathQuery
{
    ...
    bool Active;    // Used to disambiguate the case when ID == 0 and e.g. some code calls
                    // PushOverrideID(0).
...
struct ImGuiContext
{
    ...
    // - Tabbing (Tab, Shift+Tab) and Windowing (Ctrl+Tab, Ctrl+Shift+Tab) are enabled REGARDLESS of
    // ImGuiConfigFlags_NavEnableKeyboard being set.
    // - Ctrl+Tab is reconfigurable because it is the only shortcut that may be polled when no
    // window are focused. It also doesn't work e.g. Web platforms.
    bool ConfigNavEnableTabbing; // = true. Enable tabbing (Tab, Shift+Tab). PLEASE LET ME KNOW IF
                                 // YOU USE THIS.
    ...
    ImGuiKeyChord ConfigNavWindowingKeyNext; // = ImGuiMod_Ctrl | ImGuiKey_Tab (or ImGuiMod_Super |
                                             // ImGuiKey_Tab on OS X). Set to 0 to disable. For
    ...
    ImGuiID InputTextReactivateId; // ID of InputText to reactivate on next frame (for
                                   // io.ConfigInputTextEnterKeepActive behavior)
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImGuiWindowBgClickFlags
        BgClickFlags : 8; // Configure behavior of click+dragging on window bg/void or over items.
                          // Default sets by io.ConfigWindowsMoveFromTitleBarOnly. If you use this
    ...
    ImGuiWindow
        *ParentWindowForFocusRoute; // Set to manual link a window to its logical parent so that
                                    // Shortcut() chain are honored (e.g. Tool linked to Document)
  ...
  public:
    ...
    // [OBSOLETE] ImGuiWindow::CalcFontSize() was removed in 1.92.0 because error-prone/misleading.
...
IMGUI_API bool
ItemAdd(const ImRect &bb, ImGuiID id, const ImRect *nav_bb = NULL, ImGuiItemFlags extra_flags = 0);
...
IMGUI_API void
ActivateItemByID(ImGuiID id); // Activate an item by ID (button, checkbox, tree node etc.).
                              // Activation is queued and processed on the next frame when the item
                              // is encountered again. Was called 'ActivateItem()' before 1.89.7.
...
IMGUI_API void TextAligned(float align_x,
                           float size_x,
                           const char *fmt,
...
IMGUI_API void
ExtendHitBoxWhenNearViewportEdge(ImGuiWindow *window, ImRect *bb, float threshold, ImGuiAxis axis);
...
IMGUI_API void
DebugNodeTexture(ImTextureData *tex,
                 int int_id,
...
typedef ImFontLoader
    ImFontBuilderIO; // [renamed/changed in 1.92.0] The types are not actually compatible but we
...
IMGUI_API void
ImFontAtlasBuildNotifySetFont(ImFontAtlas *atlas, ImFont *old_font, ImFont *new_font);
...
IMGUI_API void
ImFontAtlasBuildSetupFontSpecialGlyphs(ImFontAtlas *atlas, ImFont *font, ImFontConfig *src);
...
IMGUI_API bool
ImFontAtlasFontInitOutput(ImFontAtlas *atlas,
...
IMGUI_API void
ImFontAtlasUpdateNewFrame(ImFontAtlas *atlas, int frame_count, bool renderer_has_textures);
...
IMGUI_API void
ImFontAtlasUpdateDrawListsTextures(ImFontAtlas *atlas, ImTextureRef old_tex, ImTextureRef new_tex);
...
IMGUI_API void
ImFontAtlasTextureBlockFill(ImTextureData *dst_tex, int dst_x, int dst_y, int w, int h, ImU32 col);
...
#ifdef IMGUI_ENABLE_TEST_ENGINE
extern void
ImGuiTestEngineHook_ItemAdd(ImGuiContext *ctx,
                            ImGuiID id,
                            const ImRect &bb,
